...
public class NouvelleLigneDeReglement {
	VBox root = new VBox();
	...
	private Consumer<Reglement> ReglementLineSelectCallBack;
	
	public void setReglementLineSelectCallBack(Consumer <Reglement> callback) {
		this.ReglementLineSelectCallBack = callback;
	}
	...
	Button AddReglementButton = new Button("Ajouter");
	...
	private void appendNodesToWindow() {
		...
		ButtonsContainer.getChildren().addAll(AddReglementButton, CancelButton);
	...
	private void addEvents() {
		CancelButton.setOnAction(event -> {
			Confirmation confirmation = new Confirmation("Quitter", "Vous êtes sûr de vouloir annuler l'ajout de la nouvelle ligne de règlement?");
		...
		AddReglementButton.setOnAction(event -> {
			if (isValidForm()) {
				Confirmation confirmation = new Confirmation("Ajouter la ligne de règlement", "Vous êtes sûr de vouloir ajouter cette ligne de règlement?");
				confirmation.setResponseCallBack(response -> {
					if (response == true) {
						Reglement reglement = new Reglement(DateInput.getValue(), Double.parseDouble(MontantTextField.getText()));
						if (TypeComboBox.getValue().equals("CHEQUE")) {
							reglement.setType("CHEQUE");
							reglement.setNumero_cheque(Long.parseLong(NumeroChequeTextField.getText()));
							reglement.setDate_echeance(DateEcheanceInput.getValue());
							reglement.setBanque(BanqueComboBox.getValue());
							reglement.setNom(NomTextField.getText());
						}
						ReglementLineSelectCallBack.accept(reglement);
						window.close();
					}
				});
			} else {
	...
	public void updateLayout(String type) {
		...
		if (type.equals("ESPECE")) {
			...
			Container.getChildren().addAll(NumeroChequeLabel, NumeroChequeTextField, 
					DateEcheanceLabel, DateEcheanceInput, 
					BanqueLabel, BanqueComboBox, 
					NomLabel, NomTextField, ButtonsContainer);
			window.setHeight(650);
		}
	}
	
	public NouvelleLigneDeReglement(Double rest) {
		this.rest = rest;
		MontantTextField.setText(rest + "");
		initWindow();
		addStylesToNodes();
		appendNodesToWindow();
		getBanques();
